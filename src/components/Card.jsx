import React from "react";
import { FaRegFileAlt } from "react-icons/fa";
import { MdOutlineFileDownload } from "react-icons/md";
import { IoClose } from "react-icons/io5";
import { motion } from "framer-motion";

const Card = ({ itemData, reference }) => {
  console.log(itemData.tag.tagColor);

  return (
    <>
      <motion.div
        drag
        dragConstraints={reference}
        whileDrag={{ scale: 1.2 }}
        dragElastic={0.1}
        dragTransition={{ bounceStiffness: 600, bounceDamping: 10 }}
        className="relative shrink-0 w-60 h-72 rounded-[45px] bg-zinc-900/90 text-white px-8 py-10 overflow-hidden"
      >
        <FaRegFileAlt />
        <p className="text-sm leading-tight mt-5 font-semibold">
          {itemData.desc}
        </p>
        <div className="footer absolute w-full bottom-0 left-0">
          <div className="flex items-center justify-between mb-5 py-3 px-8">
            <h5>{itemData.filesize}</h5>
            <span className="w-7 h-5 bg-zinc-600 rounded-full flex items-center justify-center">
              {itemData.close ? (
                <IoClose />
              ) : (
                <MdOutlineFileDownload size=".8em" color="#fff" />
              )}
            </span>
          </div>
          {itemData.tag.isOpen && (
            <div
              className={`tag w-full py-4 ${
                itemData.tag.tagColor === "blue"
                  ? "bg-blue-400"
                  : "bg-green-400"
              }  flex justify-center items-center`}
            >
              <h3 className="text-sm font-semibold">{itemData.tag.tagTitle}</h3>
            </div>
          )}
        </div>
      </motion.div>
    </>
  );
};

export default Card;
